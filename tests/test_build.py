...
from conftest import ROOT, run_python, compatibility_library
...
def test_template_build_and_check(core, template_library):
    ...
    assert entry["Info"]["aeco"] == {"version": "0.1.0", "tier": "kind", "requires": {"usdAeco": ">=0.8,<0.9"}}
...
def test_build_against_installed_core_without_bootstrap(tmp_path, core):
    ...
    source = compatibility_library("usdAecoInstalled", tmp_path / "source")
...
def test_builder_can_build_core_and_register_metadata(tmp_path, core):
    ...
    shutil.copytree(core[0] / "schemas/usdAeco", source)
    (source / "library.json").write_text(json.dumps({
        "name": "usdAeco", "version": "0.7.0", "tier": "core", "requires": {}}))
...
assert Usd.SchemaRegistry().FindAppliedAPIPrimDefinition("AecoAxisAPI").GetPropertyMetadata(
    "aeco:axis:length", "aecoDerived") is True
...
def test_builder_rejects_invalid_schema(tmp_path, core, defect):
    source = compatibility_library("usdAecoInvalid", tmp_path / "source")
...
@pytest.mark.core
def test_strict_builder_refuses_new_template_target_on_older_core(tmp_path, core):
    source = new_library("usdAecoFuture", tmp_path / "future")
    result = build("usdAecoFuture", source, tmp_path / "out", [core[1]])
    assert result.returncode != 0 and "requires usdAeco >=0.9,<1.0" in result.stderr
...
def test_flat_source_dependency_and_installed_validator_layout(tmp_path, core, template_library):
    ...
    source = compatibility_library("usdAecoFlat", tmp_path / "source")
    manifest = json.loads((source / "library.json").read_text())
